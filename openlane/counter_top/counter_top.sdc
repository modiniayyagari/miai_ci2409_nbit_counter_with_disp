...
# Reset input delay
#MIAI: Changed wb_rst_i to rst (corresponding to reset signal name in counter_top
set_input_delay [expr $::env(CLOCK_PERIOD) * 0.5] -clock [get_clocks {clk}] [get_ports {rst}]

# Multicycle paths
#MIAI: No multicycle paths in COUNTER_TOP
...
# Input delays
#MIAI: Below ports are not present in COUNTER_TOP
...
#MIAI: Adding input delay constraints for cnt_* and disp_tgl
set clk_period $::env(CLOCK_PERIOD)
set_input_delay [expr 0.8 * $clk_period] -clock [get_clocks {clk}] [get_ports {cnt_*}]
#set_input_delay [expr 0.8 * $clk_period] -clock [get_clocks {clk}] [get_ports {disp_tgl}]
...
#MIAI: Adding output delay constraints for disp_val
#set clk_period $::env(CLOCK_PERIOD)
set_output_delay [expr 0.8 * $clk_period] -clock [get_clocks {clk}] [get_ports {disp_val[*]}]
set_output_delay [expr 0.8 * $clk_period] -clock [get_clocks {clk}] [get_ports {out_en[*]}]
